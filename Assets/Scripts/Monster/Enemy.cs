...
public abstract class Enemy : MonoBehaviour
{
    ...
    public void Start()
    {
        ...
        loading = Shared.loading.GetComponent<Loading>();
    }
    ...
    public void bleeding()  // 도트데미지 주는 함수
    {
        if (bleedingTime > 0)
        {
            if (selectWeapon == 0 && Swordlevel > 0 && Enemy_HP > 0)
            {
                ...
                if (AmIBoss)
                {
                    Shared.gameMgr.GetComponent<BossHpController>().BossHit(damage);
                }
    ...
    public IEnumerator Hit(float damage) // 피해 함수
    {
        ...
        Player player = Shared.player.GetComponent<Player>();
        Ui_Controller ui = Shared.gameMgr.GetComponent<Ui_Controller>(); //윤성권 추가함
        Proficiency_ui pro = Shared.gameMgr.GetComponent<Proficiency_ui>(); // 숙련도 추가함
    ...
    void watcihingHP()      // 체력이 0이면 Die 코루틴 실행하는 함수
    {
        Player player = Shared.player.GetComponent<Player>();
        Ui_Controller ui = Shared.gameMgr.GetComponent<Ui_Controller>(); //윤성권 추가함
        Proficiency_ui proficiency_ui = Shared.gameMgr.GetComponent<Proficiency_ui>(); // 숙련도 추가함
        if (Enemy_HP <= 0 && !watching)
        {
            ...
            if (!giveReward)
            {
                proficiency_ui.GetProExp(Stage);
                ui.GetExp(Stage);
                ui.GetGold(Stage);
    ...
    void orcDie()   // Orc 보스의 죽는 애니메이션
    {
        ...
        if (Enemy_HP <= 0)
        {
            if (Dying)
            {
                Shared.player.FirstMaterial = true;
                BossSpriteBox.enabled = false;
                rigid.isKinematic = true;
                Shared.gameMgr.GetComponent<BossHpController>().BossDead();
            }
            if (!giveReward)
            {
                giveReward = true;
