...
public class Player : MonoBehaviour
{
    ...
    void Start() //추가함
    {
        movecamera = Shared.movecamera;
        DataManager dataMgr = Shared.dataMgr;
    ...
    void Update()
    {
        ...
        AxePro2();
        if (animator.GetFloat("AttackSpeed") >= 2.75f)
        {
            animator.SetFloat("AttackSpeed", 2.75f);
        }
        if (delayTime <= 0.3f)
        {
            delayTime = 0.3f;
    ...
    void Player_Move() //Player 이동, 점프
    {
        ...
        if (Input.GetKey(KeyCode.DownArrow) && !animator.GetBool("Sliding") && !animator.GetBool("Wall_slide")) //발판에서 밑으로 점프시 내려가기
        {
            if (Input.GetKeyDown(KeyCode.Space))
                StartCoroutine(PadJump(0));
        }
        else if (Input.GetKeyDown(KeyCode.Space) && !animator.GetBool("Sliding") && !animator.GetBool("Wall_slide") && JumpCnt > 0)
        {
    ...
    public void Player_Attack() //Player 공격모음
    {
        ...
        {
            if (!animator.GetBool("Sliding") && !animator.GetBool("Wall_slide") && !isMasterSkill && !isSkill && !isCharging && !isdelay && !isWall)
            {
        ...
        if (Input.GetKey(KeyCode.A) && !animator.GetBool("Sliding") && !isSkill && !isMasterSkill) // Axe 차징 공격
        {
        ...
        if (Input.GetKeyDown(KeyCode.A) && !animator.GetBool("Sliding") && !isSkill && !isMasterSkill)    //기본 공격
        {
    ...
    public IEnumerator Skill()//스킬 작동시 실행
    {
        ...
        {
            animator.SetTrigger("arrow_atk");   //애니메이션에 Bow_Attack 함수 실행이 들어가있음
        }
    }
    ...
    void OnCollisionStay2D(Collision2D collision)   // 벽 콜라이젼이 Player에 닿고 있으면 실행, 점프착지 시 콜라이젼 닿을 시 점프 해제
    {
        ...
        if (rayHitDown.collider != null && !animator.GetBool("Sliding"))
        {
            JumpCnt = JumpCount;
        }
    }
    ...
    IEnumerator Die() //Player 사망시 스프라이트 삭제
    {
        ...
        Shared.soundMgr.bgSound.clip = null;
        PlaySound("Die");
